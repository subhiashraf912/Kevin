...
export default class NightcoreCommand extends BaseCommand {
  ...
  async run(client: DiscordClient, message: Message, args: Array<string>) {
    ...
    if (search.loadType === "NO_MATCHES" || search.loadType === "LOAD_FAILED") return message.reply("No matches were found.");
    ...
    const encoderArgstoset = ["-af", "asetrate=48000*1.15,aresample=48000,aphaser"];
    ...
    stream.pipe(fs.createWriteStream(fileName)).on("finish", async () => {
      ...
      const attachment = new MessageAttachment(`./${fileName}`, `${search.tracks[0].title}.mp3`);
