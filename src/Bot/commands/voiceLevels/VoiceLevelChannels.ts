...
export default class AddVoiceLevelChannel extends BaseCommand {
  ...
  async run(client: DiscordClient, message: Message, args: Array<string>) {
    const { channels: voiceLevelsChannels } =
      await client.configurations.voiceLevels.channels.get(message.guildId!);
  ...
  generateRolesEmbed(channels: GuildChannel[]) {
    ...
    for (let i = 0; i < channels.length; i += 10) {
      ...
      const embed = new MessageEmbed().setDescription(
        `\`\`\`💢 Voice Level Channels: ${channels.length}💢\`\`\`\`\`\`⚡The channels that level system can work in!\nOther channels won't give xp!⚡\`\`\`${info}`
      );
      embeds.push(embed);
    }
    return embeds;
