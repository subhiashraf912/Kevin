...
class WelcomesManager {
  ...
  formatConfiguration(configuration: WelcomeChannelsRestOptions): WelcomesConfiguration {
    const guild = this.client.guilds.cache.get(configuration.guildId)!;
    const channel = guild.channels.cache.get(configuration.channelId) as GuildTextBasedChannel;
  ...
  async create(options: WelcomeChannelsRestOptions): Promise<WelcomesConfiguration> {
    const configuration = await this.client.database.models.welcomes.create(options);
    const createdData = this.formatConfiguration(configuration);
    this.cache.set(configuration.guildId, createdData);
    return createdData;
  }

  async update(guildId: Snowflake, options: UpdateWelcomesConfigurationOptions): Promise<WelcomesConfiguration> {
    let databaseConfiguration = await this.client.database.models.welcomes.findOneAndUpdate(
      {
        guildId,
      },
      options,
  ...
  async delete(guildId: Snowflake) {
    ...
    if (!configuration) throw new Error("There's no welcome system set for this server.");
  ...
  async fetch(guildId: Snowflake) {
    let databaseConfiguration = await this.client.database.models.welcomes.findOne({
      guildId,
    });
